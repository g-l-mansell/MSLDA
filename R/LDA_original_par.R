...
opt_phi_d <- function(phi, gamm, beta, w, K){
  beta_prime <- beta[, w]
...
opt_gamma_d <- function(phi, alpha){
...
e_step_d <- function(gamm, phi, alpha, beta, w, V, K){
  gamm <- alpha + length(w)/K
  phi <- matrix(1/K, length(w), K)

  for(iter in 1:20){
    gamm_old <- gamm

    #update phi and gamma for this document
    phi <- opt_phi_d(phi, gamm, beta, w, K)
    gamm <- opt_gamma_d(phi, alpha)
...
#find the overall log-likelihood
...
make_N <- function(docs, V){
  N <- matrix(0, length(docs), V)
  for(v in 1:V){
    N[,v] <- sapply(docs, function(w) sum(w==v))
  }
  return(N)
}
...
initalise_beta <- function(docs, K, V){
  ...
  beta <- beta / rowSums(beta) #normalise rows of beta to sum to 1
  beta[beta==0] <- 1e-16 #write over any 0s
  return(beta)
}
...
lda_original <- function(docs, K, max_iter=20, thresh=NULL, seed=NULL){
